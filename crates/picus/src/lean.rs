...
fn write_module(
    ...
    if let Some((ins, outs)) = crate::picus_builder::PORT_ORIGINS.lock().unwrap().get(&m.name) {
        ...
        // Named accessors, stable across regeneration as long as the chip's lookups are: one
        // `List F` per limb group (`mem_read[0].val` → `in_mem_read_0_val`) and one `F` per scalar.
...
"#,
    ...
    // The executable ISA model and its oracle-vector checks (hand-written / generated by
    // crates/core/executor/tests/spec_vectors/gen_lean.py) live beside the chip files.
    for m in ["Isa", "IsaVectors", "IsaDecode"] {
        if out_dir.join("ZirenDet").join(format!("{m}.lean")).exists() {
            root.push_str(&format!("import ZirenDet.{m}\n"));
        }
    }
    // Hand-written bridge theorems (chip outputs = ISA semantics) under ZirenDet/Bridge.
    ...
    if bridge_dir.exists() {
        ...
        bridges.sort();
        for b in bridges {
            root.push_str(&format!("import ZirenDet.Bridge.{b}\n"));
        }
    }
